...
const botScript = async () => {
  ...
  const response = await fetch('https://www.davidrumsey.com/luna/servlet/as/fetchMediaSearch?&lc=RUMSEY~8~1&q=what=Atlas+Map&bs=1&fullData=true&random=true');
  let map = await response.json();
  map = map[0];
  
  console.log("openig a map...");

  const imageURL = map.urlSize4 || map.urlSize3 || map.urlSize2 || map.urlSize1 || map.urlSize0;
  ...
  if (imageURL){
    try {
      ...
      const description = `${attributes.type}: ${attributes.pub_note}`;
