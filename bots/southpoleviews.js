const fs = require('fs'),
      ...
      stations = require(__dirname + '/../helpers/south-pole-stations.js'),
...
module.exports = function(){
  ...
  var station = helpers.randomFromArray(stations);
  ...
  (async () => {
    ...
    page.on('load', async response => {
      ...
      var img_src = $(`${station.element}`).attr('src'),
          img_url;
      
      if (img_src.indexOf('http') === -1){
          img_url = `${station.page_url}${img_src}`;        
      }
      else{
        img_url= img_src;
      }

      helpers.loadImage(img_url, function(err, imgData){
