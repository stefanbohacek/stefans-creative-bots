...
const pickNewCapital = () => {
    ...
    const flagUrl = `https://static.stefanbohacek.dev/images/flags/${capital.country.replace(/ /g, '_')}.png`;
...
const checkAnswer = (answer) => {
    answer = answer.trim().toLowerCase().normalize('NFD').replace(/\p{Diacritic}/gu, '');
    const correctAnswer = savedData.capital.toLowerCase().normalize('NFD').replace(/\p{Diacritic}/gu, '');
...
const getLeaderboard = () => {
    ...
    return `\n\n${
        // topThree.map((top, index) => `${medals[index]} ${top.accounts.map(a => `@${a}`).join(', ')} (${top.score} pt(s))`).join('\n')
        topThree.map((top, index) => `${medals[index]} ${top.accounts.map(a => `${a}`).join(', ')} (${top.score} pt(s))`).join('\n')
    }`;
}
...
module.exports = {
  ...
  reply: async (postID, from, messageText, fullMessage) => {
    ...
    if (fullMessage.data.status.visibility === 'public' || fullMessage.data.status.visibility === 'unlisted'){
        if (checkAnswer(messageText)){
            updateScores(from);
            reply = `Yes, ${savedData.capital} is the capital of ${savedData.country}, correct! ${getLeaderboard()}`;
            pickNewCapital();
        } else {
